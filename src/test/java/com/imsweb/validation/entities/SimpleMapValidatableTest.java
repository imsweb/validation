...
public class SimpleMapValidatableTest {
    ...
    public void testSimpleMapValidatable() throws Exception {
        ...
        secondLevel.add(Collections.<String, Object>singletonMap("prop", "1"));
        secondLevel.add(Collections.<String, Object>singletonMap("prop", "1"));
        secondLevel.add(Collections.<String, Object>singletonMap("prop", "1"));
        entity.put("level2", secondLevel);
        v = new SimpleMapValidatable("ID", "level1", entity);
        Assert.assertEquals(3, ValidationEngine.validate(v).size());

        // and finish with a third level
        Map<String, Object> map = new HashMap<>();
        List<Map<String, Object>> thirdLevel = new ArrayList<>();
        thirdLevel.add(Collections.<String, Object>singletonMap("prop", "1"));
        thirdLevel.add(Collections.<String, Object>singletonMap("prop", "1"));
        thirdLevel.add(Collections.<String, Object>singletonMap("prop", "1"));
        map.put("level3", thirdLevel);
        secondLevel.add(map);
        v = new SimpleMapValidatable("ID", "level1", entity);
        Assert.assertEquals(6, ValidationEngine.validate(v).size());
    ...
    public void testFollowCollection() throws IllegalAccessException {
        ...
        Map<String, Object> m1 = Collections.<String, Object>singletonMap("prop", "1");
        Map<String, Object> m2 = Collections.<String, Object>singletonMap("prop", "1");
        Map<String, Object> m3 = Collections.<String, Object>singletonMap("prop", "1");
    ...
    public void testForceFailureOnEntityMechanism() throws Exception {
        ...
        Rule r1 = ValidationEngine.getValidators().get("fake-validator-force-failure-on-entity").getRule("fv-unique-rule-1");
        Rule r2 = ValidationEngine.getValidators().get("fake-validator-force-failure-on-entity").getRule("fv-unique-rule-2");

        Collection<RuleFailure> failures;
        Map<String, Object> entity = new HashMap<>();

        // null collection of level2 objects -> there should be an exception in the edit
        failures = ValidationEngine.validate(new SimpleMapValidatable("ID", "level1", entity), r1.getId());
        ...
        Map<String, Object> obj1 = Collections.<String, Object>singletonMap("prop", "1");
        list.add(obj1);
        failures = ValidationEngine.validate(new SimpleMapValidatable("ID", "level1", entity), r1.getId());
        Assert.assertTrue(failures.iterator().next().getProperties().isEmpty());
        ...
        failures = ValidationEngine.validate(new SimpleMapValidatable("ID", "level1", entity), r2.getId());
        Assert.assertEquals(2, failures.iterator().next().getProperties().size());
        Assert.assertTrue(failures.iterator().next().getProperties().contains("level1.level2[0].prop"));
        Assert.assertTrue(failures.iterator().next().getProperties().contains("level1.level2[2].prop"));
