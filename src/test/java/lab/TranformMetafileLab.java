...
public class TranformMetafileLab {
    ...
    public static void main(String[] args) throws Exception {
        File metafile = new File("C:\\dev\\naaccr-translated-edits.xml");

        ValidatorContextFunctions.initialize(new MetafileContextFunctions());

        System.out.println("Loading original XML file...");
    ...
    private static void runEdits(Validator v) throws Exception {
        System.out.println("Adding original XML file...");
        ValidationEngine.initialize(v);

        System.out.println("Running original XML file...");
        Map<String, String> entity = Collections.singletonMap("nameLast", "123");
        SimpleNaaccrLinesValidatable validatable = new SimpleNaaccrLinesValidatable(Collections.singletonList(entity), null, true);
        System.out.println("  > num result: " + ValidationEngine.validate(validatable).size());
        ValidationEngine.uninitialize();
    }

    private static void translate(Validator v) throws Exception {
        System.out.println("Translating it...");
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(new File("C:\\dev\\NaaccrTranslatedEdits.groovy")))) {
